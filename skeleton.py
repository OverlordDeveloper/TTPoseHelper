...
class Skeleton:
    ...
    def _update_model_matrix(self):
        ...
        back = Matrix44.from_translation(-center)
        forth = Matrix44.from_translation(center)

        modelMat = Matrix44.identity()

        modelMat = back @ self.rotationMatrix @ forth

        
        self.modelMatrix = self.translationMatrix @ modelMat @ self.scaleMatrix
    ...
    def _calculate_center(self):
        ...
        x = (self.pose["RIGHT_SHOULDER"][0] + self.pose["LEFT_SHOULDER"][0] + self.pose["LEFT_HIP"][0] + self.pose["RIGHT_HIP"][0]) / 4.0
        y = (self.pose["RIGHT_SHOULDER"][1] + self.pose["LEFT_SHOULDER"][1] + self.pose["LEFT_HIP"][1] + self.pose["RIGHT_HIP"][1]) / 4.0
        z = (self.pose["RIGHT_SHOULDER"][2] + self.pose["LEFT_SHOULDER"][2] + self.pose["LEFT_HIP"][2] + self.pose["RIGHT_HIP"][2]) / 4.0
